...
namespace MyTraining1121AngularDemo.Customers
{
    ...
    public class CustomerAppService : MyTraining1121AngularDemoAppServiceBase, ICustomerAppService
    {
        ...
        {
            //var customerUserId = _customerUserRepository.GetAll().Select(c => c.UserRefId);

            //var users=await _userRepository.GetAll().Where(p=>!customerUserId.Contains(p.Id)).ToListAsync();
